      ...
      <section className="py-16 px-6">
        ...
        >
          <Section title="1. Who we are">
            <P>This website is operated by Lollykrown ("we", "us", "our"). We are a freelance web development and photography business based in Sunderland, UK.</P>
            <P>If you have any questions about this privacy policy or how we handle your data, please contact us at <a href="mailto:joe_kayu@yahoo.com" className="underline" style={{ color: "var(--color-accent-subtle)" }}>hello@lollykrown.xyz</a>.</P>
          </Section>
          ...
          <Section title="4. Google Analytics">
            <P>We use Google Analytics 4 (GA4), provided by Google LLC, to collect anonymised usage statistics about how visitors use this website.</P>
            ...
            <P>We have enabled IP anonymisation in Google Analytics, which means your full IP address is never stored. The data collected is aggregated and does not identify you personally.</P>
            <P>Google's data processing is governed by <a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer" className="underline" style={{ color: "var(--color-accent-subtle)" }}>Google's Privacy Policy</a>. You can opt out of Google Analytics tracking across all websites by installing the <a href="https://tools.google.com/dlpage/gaoptout" target="_blank" rel="noopener noreferrer" className="underline" style={{ color: "var(--color-accent-subtle)" }}>Google Analytics Opt-out Browser Add-on</a>.</P>
            <P>We only activate Google Analytics after you have accepted cookies via our cookie banner. If you decline, no analytics data is collected.</P>
          </Section>
          ...
          <Section title="5. Cookies">
            ...
            >
              {[
                { name: "cookie-consent", type: "Essential", purpose: "Stores your cookie preference (accepted/declined). Expires after 1 year.", provider: "This site" },
                { name: "_ga", type: "Analytics", purpose: "Distinguishes unique users. Expires after 2 years.", provider: "Google" },
                { name: "_ga_*", type: "Analytics", purpose: "Maintains session state for Google Analytics 4. Expires after 2 years.", provider: "Google" },
              ].map((c, i) => (
                <div
                  key={c.name}
                  ...
                  style={{
                    ...
                    backgroundColor: i % 2 === 0 ? "var(--color-bg-card-darker)" : "transparent",
                  }}
                >
                  <span className="font-mono text-xs font-semibold" style={{ color: "var(--color-accent-subtle)" }}>{c.name}</span>
                  <span style={{ color: "var(--color-text-muted)" }}>{c.provider}</span>
                  <span
                  ...
                  <span style={{ color: "var(--color-text-secondary)" }}>{c.purpose}</span>
                </div>
              ))}
            </div>
            <P>You can control cookies through your browser settings. Disabling all cookies may affect the functionality of this website.</P>
          </Section>
          ...
          <Section title="8. Data sharing">
            <P>We do not sell, rent, or trade your personal data. We share data only with the following third parties:</P>
            <Ul items={[
            ...
            <P>Both Google and Vercel may process data outside the UK/EEA. They do so under standard contractual clauses approved by the UK ICO.</P>
          </Section>
          ...
          <Section title="9. Your rights">
            ...
            <P>To exercise any of these rights, contact us at <Link href="mailto:joe_kayu@yahoo.com" className="underline" style={{ color: "var(--color-accent-subtle)" }}>hello@lollykrown.xyz</Link>. We will respond within 30 days.</P>
          </Section>
          ...
          <motion.div variants={fadeUp} className="flex items-center gap-4 pt-8 border-t" style={{ borderColor: "var(--color-border)" }}>
            <Link href="/terms" className="text-sm font-semibold transition-colors" style={{ color: "var(--color-accent-subtle)" }}>
              Terms of Service →
            </Link>
            <span className="text-(--color-border-hover)">|</span>
            <Link href="/" className="text-sm transition-colors text-(--color-text-secondary) hover:text-(--color-text-primary) hover:underline">
              Back to home
            </Link>
          </motion.div>
        </motion.div>
